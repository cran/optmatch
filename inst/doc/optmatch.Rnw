...
shares these advantages with full matching with restrictions \citep{hansen:klopfer:2006}, a procedure that can also mimic pair matching and
...
% \begin{description}
% \item[\texttt{
% fullmatch}]  \textrm{Optimal full matching}
% \item[\texttt{
% pairmatch}] \textrm{Optimal pair matching}
% \item[\texttt{
% matched}]   \textrm{Identification of units placed into matched sets}
% \item[\texttt{
% matchfailed}]  \textrm{Identification of units/subclasses for which matching 
% was infeasible}
% \item[\texttt{
% unmatched}]  \textrm{Identification of units not placed into matched
% sets}
% \item[\texttt{
% matched.distances}] \textrm{??}
% \item[\texttt{
% makedist}] \textrm{??}
% \item[\texttt{
% mahal.dist}] \textrm{??}
...
{\selectlanguage{english}\ttfamily
\textrm{If you adapt this example directly to your problem, then be sure to check after matching that the names of the factor that results from matching are identical with the row names of your working data frame.  Sometimes the observations get scrambled from their original order, and things get confused when you go on to do analysis.  Another way to avoid this problem is to make your distance using the} \texttt{makedist, mahal.dist} \textrm{or} \texttt{pscore.dist} \textrm{functions, which keep track of the order of observations for you.  The help page for } \texttt{makedist}, \textrm{accessible with} \texttt{?makedist},\textrm{shows in its examples how to make the distance given above using it.}}
...
( plantdist <- plantdist/(abs(datediffs)<3) )
@

{\selectlanguage{english}\ttfamily
\textrm{The infinite entries occur where the matrix} \texttt{datediffs} \textrm{has an entry less than $-3$ or greater than $+3$, indicating an unwanted match.  (If you had constructed the distance} \texttt{plantdist} \textrm{using} \texttt{makedist}, \textrm{or} \texttt{mahal.dist} \textrm{or} \texttt{pscore.dist}, \textrm{then you may have to use} \texttt{makedist} \textrm{also to create} \texttt{datediffs} \textrm{in order for the division} \texttt{plantdist/(abs(datediffs)<3)} \textrm{to work.)}}
...
F,U, and Z is the difference between U and Z: 71.08 {}- 69.50 = 1.58.
  ...
  The function \texttt{matched.distances} is also useful for determining how close matches are; see its help page for details.
}
\bigskip
...
Rosenbaum, P.~R. (1991), \enquote{A Characterization of Optimal Designs for
  Observational Studies,} \textit{Journal of the Royal Statistical Society},
  53, 597-- 610.

\bibitem[{Rosenbaum(2002)}]{rosenbaum:2002}
--- (2002), \textit{Observational Studies}, Springer-Verlag, 2nd ed.
